//参数加载
var mapOption = dataset.mapview["tianditu"];
...
if(sispcxt){
	tileUrl = sispcxt + "/tileServerProxy";
}
...
var mapParams = {
	...
	"tilenum" : 16,
	"visible" : false,
	"extent":mapOption.extent ? mapOption.extent : [ -180, -90, 180, 90 ],
};
var layerConfig = {
	"baselayer_vector" : [
...
//地图工具加载
var map_tools_olchart,map_tools_choose,map_tools_point,map_tools_ledit,map_tools_pedit,map_tools_share,map_tools_globe,map_tools_clear,map_tools_save,map_tools_mapo,map_tools_pointsearch;
var toolsMap = {};
var mainbar;
function initTools() {
	...
	map_tools_ledit = new ol.control.Toggle({
		...
		bar : new ol.control.Bar({
			controls : [ new ol.control.TextButton({
				...
				handleClick : function() {
					if (ledit.getInteraction().sketchCoords_.length <= 2) {
						$("#measureResult").html('');
					}
		...
		geom.on('change',
				function(e) {
					...
					if (area < 1000) {
						...
						$("#measureResult").html(
								(area / 10E6).toFixed(2) + "十万亩");
					} else {
						$("#measureResult").html(
								(area / 10E7).toFixed(2) + "百万亩");
					}
...
function loadPieChartLayer(layername,dataset,piedataset){
...
function loadClusterLayer(layername,dataset,dataparams){
	//var dataset = 'queryCluter';
	if(dataset){
		var opt = {};
		opt.name = layername;
...
function loadHeatmapLayer(layername,dataset,dataparams){
	//var dataset = 'queryCluter';
	if(dataset){
		var opt = {};
		opt.name = layername;
...
function loadPoiLayer(layername,dataset,dataparams){
	//var dataset = 'queryCluter';
	if(dataset){
		var opt = {};
		opt.name = layername;
		opt.url = cxt + "/datainterface/getdata/list/" + dataset;
		console.log(dataset);
		pointPoi(dataset,layername);
	}
}


function pointPoi(dataset,layername){
	$.ajax({
		type: "POST",
		dataType: "json",
		...
		success: function(data) {
			...
			if(data!=null&&data.data!=null){
				var _src = cxt+"/images/marker0.png";
				var coord = [];
				...
				//图片大小
				iconStyle.getImage().setScale(0.7);
			
				var iconFeature =null;
				var features=[];
				for(var i = 0 ; i < data.data.length ; i ++){
					coord=[data.data[i].X,data.data[i].Y]
