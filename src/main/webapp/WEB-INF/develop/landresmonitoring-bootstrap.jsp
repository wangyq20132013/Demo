...
<link rel="stylesheet" type="text/css"
	href="<c:url value='/resources/bootstrap/css/bootstrap.css'/>"></link>
<link rel="stylesheet" type="text/css"
	href="<c:url value='/resources/bootstrap/css/sweetalert.css'/>"></link>
<link rel="stylesheet" type="text/css"
	href="<c:url value='/resources/bootstrap/css/bootstrap-table.css'/>"></link>
<link rel="stylesheet" type="text/css"
	href="<c:url value='/resources/H-ui/Hui-iconfont/1.0.8/iconfont.css'/>" />
<link rel="stylesheet" type="text/css"
	href="<c:url value='/develop/${appname}/index/index.css'/>" />
	<script
...
<body>
	...
	<div class="container-fluid">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="#">
				<!-- 
				<img src="${pageContext.request.contextPath}/develop/${appname}/image/logo.png" />&nbsp;
				 -->
				<span style="font-size: 32px;">${app.SYSNAME}</span>
			</a>
		</div>
			...
			<ul class="nav navbar-nav navbar-right">
				<c:if test="${not empty navitems}">
					<c:forEach items="${navitems.children}" var="nav">
							<li class="dropdown">
								<a href="#" id="${nav.ID}"  data-toggle="dropdown" aria-haspopup="true"
								url="${nav.HREF}?pvgresid=${nav.ID}" isgroup="${nav.ISGROUP}"
								isActive='${nav.ISACTIVE}'> <img
									src="${pageContext.request.contextPath}${nav.ICON}" /></br>
									${nav.NAME}
								</a>
								<c:if test="${not empty nav.children}">
									<ul class="dropdown-menu" role="menu" aria-labelledby="${nav.ID}">
										<c:forEach items="${nav.children}" var="nav">
											<li class="">
												<a href="#" id="${nav.ID}"
													url="${nav.HREF}?pvgresid=${nav.ID}" isgroup="${nav.ISGROUP}"
													isActive='${nav.ISACTIVE}' isChildren="0">
												${nav.NAME}
												</a>
											</li>
										</c:forEach>
		...
		<div class="exit">
			<span class="welcome">
				...
				<div class="dropdown" style="width: auto; float: left;">
					<a id="dLabel" href="#" data-toggle="dropdown" aria-haspopup="true">【<img src="${pageContext.request.contextPath}/develop/${appname}/image/usericon.png" style="width:15px;height:15px;"/>${user.logintitle}】</a>
					<ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
						<li><a href="#"
							onclick="popupPage('update', '个人信息', 'userinfo', 'modal', {}, 50, 50)">用户个人信息</a>
						</li>
						<li><a href="#"
							onclick="popupPage('extend', '修改密码', 'updatePwd', 'modal',{},50,50)">修改密码</a>
						</li>
						<li><a href="#" onclick="logoutSys()">退出</a></li>
					</ul>
				</div> <a href="#" onclick="logoutSys()">【退出】</a>
			</span> <span id="currentTime" class="time"></span>
		</div>
	</div>
	<!-- /.container-fluid --> </nav>

	<div class="main_iframe">
		<iframe id="main" name="main" scrolling="yes" frameborder="0" src=""
	...
	<script
		src="<c:url value='/resources/bootstrap/js/bootstrap-paginator.js' />"></script>
	<script
	...
	<script
		src="<c:url value='/develop/${appname}/index/index.js' />"></script>
