...
{	options = options || {};
	...
	$("<button>").html(options.html || "")
				.attr('title', options.title).attr("name",options.name)
				.on("touchstart click", function(e)
				{	if (e && e.preventDefault) e.preventDefault();
					if (options.handleClick) options.handleClick.call(self, e, options);
				})
				.appendTo(element);
	ol.control.Control.call(this, 
	{	element: element.get(0),
...
{	var options = opt_options || {};
	...
	{	element = $("<div>").addClass('ol-overview ol-unselectable ol-control ol-collapsed');
		if (/top/.test(options.align)) element.addClass('ol-control-top');
		if (/right/.test(options.align)) element.addClass('ol-control-right');
		$("<button><i class='"+((options.labelClass)?options.labelClass[0]:"fa fa-toggle-left")+"'>").on("touchstart", function(e){ self.toggleMap(); e.preventDefault(); })
					.click (function(){self.toggleMap();$(this).find('i').toggleClass(((options.labelClass)?options.labelClass.toString().replace(","," "):"fa-toggle-left fa-toggle-right"));})
					.appendTo(element);
		this.panel_ = $("<div>").addClass("panel")
					.appendTo(element);
		element[0].title = options.title || "Hawkeys";
	}
	...
	{	controls: new ol.Collection(),
		...
		view: new ol.View
			({	zoom: 14,
				center: options.center || [270148, 6247782],
				projection: options.projection || new ol.proj.Projection({
					code : "EPSG:3857",
					units : 'm'
				}) 
			}),
...
{	var options = opt_options || {};
	...
	this.set('position', options.position || 1);
	...
	{	// Force download on HTML5
		...
		{	var self = this;
			...
			{	// Get extension in the download
				var ext = $(this).attr("downmap").split('.').pop();
				if (ext=='jpg') ext = 'jpeg';
