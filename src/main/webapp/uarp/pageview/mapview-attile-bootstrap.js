...
function readMapOption(mapOption){
	...
	if(!mapOption.type){  //兼容json配置
		...
		_mapOption.view.minZoom = mapOption.minZoom;
		_mapOption.view.maxZoom = mapOption.maxZoom;
		...
		_mapOption.maps = [];
		var map = {};
		map.layertype = mapType;
		map.wmtsurl = mapOption.wmtsurl;
		map.wmsurl = mapOption.wmsurl;
		map.attileurl = mapOption.attileurl;
		map.layerurl = mapOption.layerurl;
		map.layernames = mapOption.layernames;
		...
		_mapOption.tools = mapOption.tools;
		_mapOption.exttools = mapOption.exttools;
		_mapOption.popupText = mapOption.popupText;
...
function initView(){
	...
	var tianditu_viewOption = function(viewOption){
		...
		option.zoom = viewOption.zoom ? viewOption.zoom : 4;
	...
	var attile_viewOption = function(viewOption){
		var option = {};
		...
		option.center = viewOption.center ? viewOption.center : [ 95, 30 ];
		option.zoom = viewOption.zoom ? viewOption.zoom : 1;
		option.maxZoom = viewOption.maxZoom ? viewOption.maxZoom : 9;
		option.minZoom = viewOption.minZoom ? viewOption.minZoom :1;
	...
	if(type == 'gwc'){
		...
		view = new ol.View(attile_viewOption(viewOption));
	}
...
function initmapContainer(number){
	...
	if(number == 1){
		var $map = $("<div id='map' class='map onemap'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		$("#mapview").find(".map").remove(".map").end().append($map);
		return ['map'];
	}else if(number == 2){
		var $left_map = $("<div id='left_map' class='map left_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		var $right_map = $("<div id='right_map' class='map right_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		...
		var $top_left_map = $("<div id='top_left_map' class='map top_left_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		var $top_right_map = $("<div id='top_right_map' class='map top_right_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		var $bottom_left_map = $("<div id='bottom_left_map' class='map bottom_left_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
		var $bottom_right_map = $("<div id='bottom_right_map' class='map bottom_right_map'></div>").css({
			"background-color":"rgb(218, 245, 253)",
			"background-image": "url('"+cxt+"/uarp/pageview/image/blankimg_zh.png')"
		});
...
function initMap(mapId) {
	...
	if(mapOption.maps){
		...
		if(mapId){
			...
			mapId_Arr = initmapContainer(mapOption.mapNumber);
		}
		mapObject.mapId_Arr = mapId_Arr;
		
		for(var i = 0;i < mapId_Arr.length;i++){
			var map = new ol.Map({
				target: mapId_Arr[i],
			...
			for(var j = 0;j < layernames.length;j++){
				...
				if(!map_layer){
					...
					}else if(layertype == "attile"){
						map_layer = getXYZImageLayer(layerUrl, layernames[j],originFlag);
					}else if(layertype == "wms"){
						map_layer = getWmsLayer(layerUrl, layernames[j],originFlag);
					}
	...
	return (Object.getOwnPropertyNames(_map).length == 1?_map[Object.getOwnPropertyNames(_map)[0]]:_map);
}

function getWmsLayer(wmsUrl, wmsLayer, params) {
...
function getWmtsLayer(wmtsUrl, wmtsLayer, params){
		...
		source : new ol.source.TileImage({
			...
			tileUrlFunction : function(tileCoord) {
				return url.replace('{z}',
						(tileCoord[0] + 1).toString()).replace('{x}',
						tileCoord[1].toString()).replace(
						'{y}',(Math.pow(2, tileCoord[0]) + tileCoord[2])
								.toString());
			}
		})
	});
